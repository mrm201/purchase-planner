"""
Enhanced Streamlit App with Editable Purchase Plan - Fixed Version
Replace your app.py with this code
"""
...
if run_btn:
    try:
        with st.spinner("Loading data and generating plan..."):
            ...
            # Load optional files - with proper error handling
            optional_loaders = [
            ...
            for file_obj, fname, loader in optional_loaders:
                try:
                    if file_obj:
                        ...
                        try:
                            ...
                            pass  # Optional file not found, continue
                except Exception as e:
                    st.warning(f"Could not load {fname}: {str(e)}")
            ...
            if forecasts and hasattr(forecasts[0], '__dict__'):
                st.session_state['forecasts'] = [vars(f) for f in forecasts]
            else:
        ...
        st.info("💡 Make sure your data files are in the specified directory or upload them above")
    except Exception as e:
...
if st.session_state['forecasts']:
    ...
    with col4:
        valid_covers = df[df['stock_cover_months'] != float('inf')]['stock_cover_months']
        avg_cover = valid_covers.mean() if len(valid_covers) > 0 else 0
    ...
    if edit_enabled:
        ...
        column_config = {
            ...
            'in_transit': st.column_config.NumberColumn('Transit', width='small', format="%d"),
            ...
            'ending_stock_after_order': st.column_config.NumberColumn('End', width='small', format="%d"),
    ...
    st.info("👆 Configure settings in the sidebar and click **Generate Plan** to start.")
    ...
    with st.expander("📖 Quick Start Guide", expanded=True):
        ...
        2. **Data Files** (place in `data/` directory):
           - ✅ Required: sales_history.json, item_parameters.json, current_inventory.json, sales_forecasts_n12.json
           - Optional: promotional_calendar.json, supplier_reliability.json, price_forecasts.json, demand_variability.json, volume_discounts.json
        
        3. Click **Generate Plan**
        
        4. **Edit Orders** (if enabled):
           - Only months >= Current Month + 2 are editable
           - Change quantities directly in the table
           - Click "Apply Changes" to save
        
        5. **Export** your plan as JSON
        
        ### Features
        - 📊 Real-time editing
        - 🔒 Smart locking (current + next month)
        - ⚠️ Risk indicators
        - 💰 Cost optimization
